...
Crate.prototype.create = function (model, data, callback) {
    var keys = [],
        values = [],
        sql = 'INSERT INTO ' + changeCase.snakeCase(this.tableEscaped(model)),
        id = null;
    for (var k in data) {
        if (k === 'id') {
            id = data[k];
        ...
        keys.push(changeCase.snakeCase(k));
    ...
    self.query(sql, function (err, info) {
        ...
        self.query('REFRESH TABLE ' + changeCase.snakeCase(self.tableEscaped(model)), function(err, info) {
            if (err) {
                callback(err);
                return;
            }
...
Crate.prototype.fromDatabase = function (model, data) {
  ...
  var json = {};
  for (var p in props) {
    var key = changeCase.snakeCase(this.column(model, p));
    var val = data[key];
    if (val === undefined) {
      continue;
    }
...
Crate.prototype.getColumns = function (model, props) {
  ...
  var names = keys.map(function (c) {
    return changeCase.snakeCase(self.columnEscaped(model, c));
  });
  return names.join(', ');
};
...
Crate.prototype._buildWhere = function (model, conds) {
  ...
  var cs = [];
  Object.keys(conds).forEach(function(key, index, array) {
    conds[changeCase.snakeCase(key)] = conds[key];
    delete conds[key];
  });
...
Crate.prototype.all = function all(model, filter, callback) {
  ...
  var sql = 'SELECT ' +
            ...
            ' FROM ' +
            changeCase.snakeCase(this.tableEscaped(model));
...
SqlConnector.prototype.dropTable = function (model, cb) {
  this.command('DROP TABLE ' + changeCase.snakeCase(this.tableEscaped(model)), cb);
};
...
Crate.prototype.propertiesSQL = function (model, withPks, withGenerated) {
  ...
  Object.keys(this._models[model].properties).forEach(function (prop) {
    ...
    var colName = changeCase.snakeCase(self.columnEscaped(model, prop));
